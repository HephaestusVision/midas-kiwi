...
bool ReceiveScene(vesKiwiPVRemoteRepresentation::vesInternal* selfInternal, const std::stringstream& resp)
{
  ...
  selfInternal->RemoteBackground1 = vesVector3f(client.backgroundColor()[0], client.backgroundColor()[1], client.backgroundColor()[2]);
  selfInternal->Lock->Unlock();
  ...
  for (size_t i = 0; i < datasets.size(); ++i) {
    ...
    else if (dataset->m_numberOfVerts == 0) {
      continue;
    }
...
void vesKiwiPVRemoteRepresentation::willRender(vesSharedPtr<vesRenderer> renderer)
{
  ...
  if (this->Internal->NewRemoteScene) {
    ...
    renderer->setBackgroundColor(this->Internal->RemoteBackground1[0],this->Internal->RemoteBackground1[1],this->Internal->RemoteBackground1[2]);
